...
function PaymentChannel(props: { paymentData: PaymentData, onChange: (d: PaymentData) => void, index: number, field: string }) {
  const label = "Canale transazione " + props.field + " per indxe=" + props.index
  ...
  return (
    <div>
      ...
      >
        {values.map(v => {
          return (
              <MenuItem key={v.key} value={v.value}>{v.icon}{" " + v.value}</MenuItem>
          )
        })}
      </Select>
    </div>
